...
UART::UART(HardwareSerial& serial,SystemStatus& systemstatus,LogController& logcontroller,uint8_t id,std::string name) :
...
void UART::sendPacket(RnpPacket& data) 
{
    ...
    if (dataSize + _sendBuffer.size() > _info.sendBufferSize){
        // not enough space
        _systemstatus.newFlag(SYSTEM_FLAG::ERROR_SERIAL,"UART Send Buffer Overflow!");
        ++_info.txerror;
        return;
    }
...
void UART::checkSendBuffer(){ // hmm this seems to be causing lag issues if a packet is split, i suspect the send buffer erase
    ...
    while (_serial.available() > 0)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    _systemstatus.newFlag(SYSTEM_FLAG::ERROR_SERIAL,"UART Receive Buffer Overflow!");
                }
            }
        }
